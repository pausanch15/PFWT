...
def cortar_fibra(fibra): # la fibra con el thin ya hecho
    ...
    convoled_fibra = cf[1:-1,1:-1] * fibra # multiplico por la fibra para que quede 'thin'
    
    bordes = np.array(np.where(convoled_fibra == 2)) #saco los bordes como los puntos que sean 2
    b1,b2 = list(bordes[:,0]),list(bordes[:,1]) #asumo que va a haber solo dos bordes siempre
    b1.reverse()
    b2.reverse()
    ...
    for i in range(len(nudos[0])):
        a = list(nudos[:,i])
        convoled_fibra[a[0],a[1]] = 0  #saco los nudos de la fibra para que quede partida
    
    bin_con_fibra = convoled_fibra > 0 #vuelvo a convertir la imagen en binaria
...
def crear_fibra(n=4,bins=50):
    n = 4
    x,y = np.random.rand(n)*50+25,np.random.rand(n)*50+25
    #uno puntos random
    spl, u = splprep([x,y],s=0)
    t_spl = np.linspace(0,1,100000)
    spline = splev(t_spl,spl)
    # creo la imagen
    im_fibra, xedges, yedges = np.histogram2d(spline[1],spline[0], bins=bins)
    im_fibra = np.pad(im_fibra, pad_width=30, mode='constant', constant_values=0)
    im_fibra = im_fibra>0 
    return im_fibra
