...
Para distinguirlos la idea es usar la función _label_ junto con _regionprops_ para sacar algún valor que permita distinguir la fibra. Para estas imágenes encontramos tres posibles propiedades que pueden ser utiles (en cursiva la descripción de _regionprops_):
    * _Excentricidad_: _Eccentricity of the ellipse that has the same second-moments as the region. The eccentricity is the ratio of the focal distance
    (distance between focal points) over the major axis length._ Para esta fibra funcionó muy bien, permite distinguir la región de la fibra de los ruidos casi siempre, dado que daba un valor de 0.999 aprox. Sin embargo, es posible que esto sea porque está fibra es muy recta y que no funcione muy bien para fibras más curvadas. Hay que probarlo
  * _Número de Euler_: _Euler characteristic of region. Computed as number of objects (= 1) subtracted by number of holes (8-connectivity)._ Para la fibra tuvo siempre un valor de 0 o 1. Habría que ver para fibra que se cruzan a si mismo si cambia. Aunque sea poco frecuente, a veces el ruido también puede dar un valor de 0 o 1. 
   * _Coeficientes de area_: No es una propiedad de _regionprops_. Es calcular el área de la _bbox_ divido el área de la región. Pareciera prometedor, aunque a veces puede ser un poco ajustado el margen entre fibra y ruido (por lo general no igual).

Una última observación es que a veces se mezcla un poco de ruido en la fibra, lo cual lleva a tener pequeñas "ramas" que se desprenden de lo que es la fibra. Esto a veces hace confundir al algoritmo, llevando a que la reconstrucción no sea lo que debe ser. UPDATE: pareciera que usar _skeletonize_ en vez de _thin_ resuelve esto último un poco. Sigue pasando pero mucho menos)

Pequeña tangente: encontramos el llamado filtro sato (sato tubeness filter) que permitiría distinguir la fibra super fácil (además de que quedaría bastante suave), pero tarda mucho tiempo en correr. Para hacerlo viable habría que aplicarlo a una pequeña región donde se encuentre la fibra. Para encontrar esa región no sé que como habría que hacerlo para que usar este filtro tenga sentido.

## Código FTP
Tenemos que poner a punto el código de FTP. El mismo está optimizado para analizar imágenes en una dimensión, pero nosotros necesitamos considerar ambas.

Algo que notamos es que en el labo nos olvidamos de medir la profundidad del agua, y de tomar alguna referencia para saber el área de la región de las imágenes que analizamos, así que queda para la próxima adquirir estos datos para poder terminar el análisis preliminar de FTP.

## Más Trabajo en el Laboratorio
Hoy volvimos al laboratorio. Esta vez sí pusimos a punto la cuba de agua. Para esto, la desagotamos por completo, y juntamos los restos de dióxido de titanio que habían decantado. Además limpiamos las paredes de la cuba, donde también se había acumulado la sustancia. Usamos una paleta de plástico y una escobita como las que se usan para limpiar vidrios.

Una vez limpia la cuba, volvimos a llenarla de agua, esta vez de agua destilada. Ya alcanzada una altura que nos pareciera correcta (no necesitamos mucha profundidad), agregamos dióxido de titanio. Sabemos que tenemos que poner como mucho 3 gramos de la sustancia por litro de agua. Estimamos tener unos 50 litros de agua en la cuba, por lo que como mucho podemos agregar 150 gramos de la sustancia. No lo pesamos, pero asumimos que alcanzamos un contraste aceptable agregando entre 100 y 75 gramos.

Pasamos a tomar mediciones nuevamente. Primero nos quedamos con 300 imágenes del patrón quieto de referencial, y luego con 3 de un cilindro (obstáculo que suelen usar en el laboratorio) cuyo diámetro medimos (8.2 cm) para tener una idea de las dimensiones en las imágenes. Estas últimas imágenes fueron tomadas con el parón proyectado. Luego agregamos un segundo cilindro igual al primero, para poder hacer un poquito de estadística (con dos cilindros...) al obtener las dimensiones de las imágenes.
