...
Para encontrar la fibra en cada imagen, diferenciarla del ruido de fondo y poder pasarla por el algoritmo que la interpola, usamos la función de la librería **skimage**: _remove_small_objects_ ([title](https://scikit-image.org/docs/dev/api/skimage.morphology.html#skimage.morphology.remove_small_objects)). De esta forma, basta con pedirle a la función que quite del array original todos aquellos objetos de menor tamaño que la fibra, y así nos deshacemos del ruido.
